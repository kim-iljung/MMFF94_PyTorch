...
MMFF_REMOTE_SOURCES = {
    "MMFF94.ff": "https://raw.githubusercontent.com/rdkit/rdkit/master/Data/MMFF94.ff",
    "MMFF94S.ff": "https://raw.githubusercontent.com/rdkit/rdkit/master/Data/MMFF94S.ff",
}


def _iter_candidate_paths(pattern: str) -> Iterable[Path]:
    """Yield plausible locations for the MMFF parameter files."""

    env_override = os.environ.get("MMFF_PARAMETER_PATH")
    if env_override:
        override = Path(env_override)
        if override.is_file():
            yield override
        elif override.is_dir():
            yield override / pattern

    # RDKit exposes two helper attributes that typically cover conda and wheel
    # installations.  Both values may point to the same directory but we keep
    # them separate here to make the search order explicit.
    rdkit_data = Path(RDConfig.RDDataDir)
    yield rdkit_data / pattern

    rdkit_base = getattr(RDConfig, "RDBASE", None)
    if rdkit_base:
        yield Path(rdkit_base) / "Data" / pattern

    # Some downstream environments strip the share/ data directory from the
    # installation to save space.  When that happens the files can still be
    # located alongside the ``rdkit`` Python package.
    try:
        rdkit_pkg = importlib.import_module("rdkit")
    except ModuleNotFoundError:
        rdkit_pkg = None
    if rdkit_pkg and hasattr(rdkit_pkg, "__path__"):
        for path in rdkit_pkg.__path__:
            candidate = Path(path) / "Data" / pattern
            yield candidate

    # Fall back to data files bundled with this package.  This allows the
    # project to remain functional even when RDKit is unavailable at runtime
    # (for example during documentation builds) as long as the parameter files
    # have been vendored.
    local_data = Path(__file__).resolve().parent / "data" / pattern
    yield local_data


def _maybe_download_parameter_file(pattern: str) -> Optional[Path]:
    """Download the requested parameter file from the RDKit repository."""

    url = MMFF_REMOTE_SOURCES.get(pattern)
    if not url:
        return None

    target = Path(__file__).resolve().parent / "data" / pattern
    if target.is_file():
        return target
    ...
    try:
        ...
        raise FileNotFoundError(
            "Automatic download of MMFF parameters failed. "
            "Please place the RDKit data file in 'mmff/data' or set "
            "MMFF_PARAMETER_PATH to point to an existing copy."
        ) from exc
...
def _find_mmff_parameter_file(mmff_variant: str = "MMFF94") -> Path:
    ...
    searched: List[Path] = []
    for candidate in _iter_candidate_paths(pattern):
        searched.append(candidate)
        if candidate.is_file():
            return candidate
    downloaded = _maybe_download_parameter_file(pattern)
    if downloaded and downloaded.is_file():
        return downloaded

    formatted = "\n".join(f"  - {path}" for path in searched)
    raise FileNotFoundError(
        "Unable to locate the MMFF parameter file.\n"
        f"Searched for '{pattern}' in:\n{formatted}\n"
        "Set the 'MMFF_PARAMETER_PATH' environment variable to point to a "
        "directory or specific file if the parameters are stored elsewhere."
    )
