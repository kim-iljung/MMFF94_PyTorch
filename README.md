...
- Optional RDKit integration for automatic system building

## Installation

The package is self-contained and only depends on PyTorch for its core
functionality.  RDKit is optional but recommended when building systems from
structure files.

```bash
pip install -e .
pip install rdkit-pypi  # optional, enables RDKit helpers
```
...
# Exclude bonded and 1-3 pairs from the non-bonded list in your own workflow.
pairs = [
    (1, 3), (1, 4),
    (2, 4), (2, 3),
    (3, 4)
...
### Building from an RDKit molecule

```python
from rdkit import Chem
from mmff import builder

mol = Chem.MolFromMolFile("ethanol.mol", sanitize=False)
forcefield = builder.forcefield_from_file("ethanol.mol")

# Alternatively, construct from an in-memory RDKit molecule
mol = Chem.AddHs(mol)
forcefield = builder.build_mmff_forcefield(mol)
```

The builder automatically assigns atom types, creates the bonded topology, and
computes Gasteiger partial charges so that you can immediately evaluate
energies with the returned :class:`~mmff.forcefield.MMFFForceField` instance.
